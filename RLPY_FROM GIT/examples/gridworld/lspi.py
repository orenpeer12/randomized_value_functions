...
def make_experiment(params, exp_id=1, path="./Results/Temp"):
    """
    ...
    policy = eGreedy(representation, epsilon=params['epsilon'])

    # Agent
    opt["agent"] = LSPI(params, policy, representation, domain.discount_factor,
                        opt["max_steps"], 300)
...
if __name__ == '__main__':
    ...
    title = f"$\epsilon-greedy$ exploration, $\epsilon={params['epsilon']}$"
    # title = "b noise"
    path = "./Results/Temp/{domain}/{agent}/{representation}/"
    experiment = make_experiment(exp_id=1, params=params, path=path)
